...
class roomimage:
    ...
    def filter(self,center_of_mass,dT):
        if center_of_mass is not None:
            ...
            if len(self.measurements) > 6:
                initstate = [self.measurements[0,0], self.measurements[0,1], (self.measurements[0,1] - self.measurements[0,0]) / dT,
                ...
                transistionCov = 1.0e-5 * np.eye(4)
                ...
                center_of_mass = filtered_state_means[0,:2].astype("int16")
    ...
    def draw(self, swc1, swc2, center_of_mass, office_activity, cab_activity, door_activity):
        if center_of_mass is not None:
            ...
            for idx in range(1, len(self.measurements) - 1, 1):
        ...
        if self.curstate < 4:
            cv2.circle(self.image, self.statemarkings[self.curstate][0], 30, (25,255,255), thickness=-1)
